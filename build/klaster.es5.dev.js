...
var k_structure = {
    ...
    'view': {
        ...
        'render': function render(tplVars, tplName) {// template render function ... here you can add template engine support for twig, etc.
        },
    ...
    var dom = {
        ...
        'getName': function getName($el) {
            if ($el.nodeType !== 3) return $el.getAttribute(dom.nameAttr($el));
        },
         ...
         * @returns {dom}
         */
        'toggleOmit': function toggleOmit($el) {
    ...
    function getValue($el, multiple) {
        ...
        if (typeof dom.multipleValues[$el.getAttribute('type')] !== 'function' || $el.getAttribute(api.multiple.attr) === "false") {
            return value.call($el);
        }

        if (multiple || $el.getAttribute(api.multiple.attr) === "true" || typeof dom.multipleValues[$el.getAttribute('type')] === 'function') {
    ...
    function getValues(type) {
        if (typeof dom.multipleValues[type] === 'undefined' && this.getAttribute(api.multiple.attr)) {
     ...
     */
    dom.multipleValues = {
        "checked": function checked($el, $elements, single) {
            if ($el.getAttribute(api.multiple.attr) === 'false' || single || document.querySelectorAll('[' + dom.nameAttr($el) + '="' + dom.getName($el) + '"]').length === 1) return $el.checked;
        ...
        "data-multiple": function dataMultiple() {
    ...
    dom.getFieldView = function (fieldN, getname) {
        ...
        if (typeof dom.child.view.views[fieldN] === 'undefined') {
            if (fieldN.includes('[')) {
                ...
                if (typeof finestMatch !== 'undefined' && finestMatch) finestMatch = finestMatch.pop();
    ...
    dom.normalizeChangeResponse = function (change) {
        ...
        if (change.substr(0, 1) !== '[') return change;
    ...
    */
    ...
    dom.setHtmlValue = function (decorated) {
        ...
        if (typeof decorated === 'undefined') decorated = '';
    ...
    */
    dom.getSelector = function (name, escapeit) {
        if (escapeit) name = name.replace(/\[/g, '\[').replace(/\]/g, '\]');
    ...
    dom.isHtmlList = function ($scope) {
        return $scope.getAttribute(api.view.attr) && $scope.getAttribute(api.view.attr).includes(api.view.value.definition.iterate);
    };
    /**
...
/******/);;var k_data = function ($) {
    ...
    */
    ...
    data._getParentObject = function (notation, ns) {
        if (typeof ns === 'undefined') ns = 'data.field.';
        var parent = false;
        if (!notation) return parent;
    ...
    data._delete = data.delete = function (notation) {
        if (typeof data['field'][notation] === 'undefined' && notation.includes('[')) {
            try {
                var parent = data._getParentObject(notation);
                eval('if(Object.prototype.toString.call(' + parent + ') === \'[object Array]\' ){' + parent + '.splice(' + parent + '.indexOf(data[\'field\'].' + notation + '),1);} else {if(typeof data[\'field\'].' + notation + '!== \'undefined\') delete data[\'field\'].' + notation + ';}');
                /**
                 * to keep indexes
    ...
    data.changed = function (field) {
        ...
        var compare = function compare(fieldName) {
            if (typeof data._modelprechange[fieldName] === 'undefined') {
                return true;
            } else {
                if (data._modelprechange[fieldName] != data['field'][fieldName]) return true;
            }
            return false;
        };
        if (typeof field !== 'undefined') {
            ...
            if (data._modelpresize !== modelsize) return true;
        }
    ...
    data.jsonPatchToObjectAccess = function (diff) {
        var path = '[' + diff.path.substr(1).split('/').join("][") + ']';
        var normal = $.normalizeChangeResponse(path);
    ...
    data.getChangedModelFields = function () {
        return data.compareJsonPatch(data._modelprechangeReal, data.field);
    };
...
(function (structure, docapi, dom, model) {
    ...
    function klaster(child) {
        ...
        cls.preRenderView = function ($field, item) {
            if (typeof model.get(dom.getName($field)) === 'undefined' || $field.getAttribute(api.view) === "__static") return false;

            if (!$field.getAttribute('data-filter')) return true;
        ...
        cls.pre_trigger = function (e) {
            ...
            if (typeof child.pre_trigger !== "undefined") return child.pre_trigger.call(_this, e);
            return true;
        };
        ...
        cls.post_trigger = function (e, result) {
            ...
            if (result != modelState || model.changed(name)) {
                ...
                cls.recognizeChange.setup.call(_this);

                model.updateValue.call(_this, result, model.field[name]);

                if (typeof child.post_trigger !== "undefined") child.post_trigger.call(_this, e, child);

                cls.model2View.call(_this);
            }
            return true;
        };
        ...
        cls.validate = function (name, value, type) {
            ...
            function validate(validateResult) {
                ...
                if (typeof value.getName === "function") cls.model2View.call(value);
        ...
        function _set($html) {
            ...
            if (typeof $html !== 'undefined' && $html.nodeType !== 3) cls.bind(this);
        ...
        cls.getDecoValPrimitive = function ($scope, scopeModelField) {
            ...
            var DecoValPrimitive = scopeModelField;
        ...
        cls.updateHtmlList = function ($scope, field, change) {
            ...
            function addListElement() {
                ...
                for (index in field) {
                    ...
                    var name = dom.getName($scope); //get child by name

                    var _$child = $scope.querySelector('[data-name="' + name + '[' + index + ']"]');

                    if (cls.preRenderView($scope, field[index])) {
                        //check filters or other stuff that could avoid rendering that item
                        ...
                        if (_$child) {
                            _$child.parentNode.replaceChild($html, _$child);
                        } else if ($close) {
                        ...
                        if (_$child) _$child.parentNode.removeChild(_$child);
                    }
            ...
            function killListElement() {
                ...
                Array.prototype.forEach.call($scope.childNodes, function (el, i) {
                    ...
                    if (typeof Elname === 'undefined') {
                        ...
                        if (!model.get(Elname) || typeof field[name] === 'undefined' || !cls.preRenderView($scope, field[name])) {
            ...
            function decision(change) {
                ...
                if (change[1] === 'value') {
                    ...
                    var _notation = change[0]; //get field that has changed

                    var scopedField = field;
                    var myChangedField = model.get(_notation);
                    var _index = /\[(.*?)\]/gi.exec(_notation)[1];

                    if (typeof field.indexOf !== 'undefined') {
                        // array
                        scopedField = typeof field[_index] !== 'undefined' ? _index : field.indexOf(change[3]); //get index of item that has chnaged
                    } else {//object
                    ...
                    if (typeof myChangedField !== 'undefined' && cls.preRenderView($scope, scopedField)) {
                        $html = dom.parseHTML(cls.view.views[viewName].call(cls.view, scopedField, _index, $scope)); // render subitem
                        ...
                        // value changed to undefined or filter does remove element
        ...
        cls.updateHtmlElement = function ($scope, scopeModelField, changed) {
            ...
            if (typeof error === 'undefined' || error.result || typeof error !== 'undefined' && dom.getView($scope) !== error.view) {
                // kein fehler aufgetreten
                if (cced !== scopeModelField) {
                    // cached value of field != model.field value
                    var decoratedFieldValue = cls.getDecoValPrimitive($scope, scopeModelField);
                    _set.call($scope, decoratedFieldValue); // bind html 
                }
            } else {
                // field view was defined i a validator is it gets rendered also if value is not in model and by that equal to undefined
        ...
        cls.model2View = function () {
            ...
            local.eachViewRepresentation = function (cnt, change, foundRepresentation, ready) {
                return function (el) {
                    ...
                    if (dom.isPrimitiveValue($scope)) {
                        //if dom view element is of type primitive
            ...
            dom.findUntilParentExists = function (notation, matches) {
                ...
                if (cnt === 0) {
                    if (model._getParentObject(notation, '') === "") return match;
            ...
            dom.updateAllViews = function (changes) {
                ...
                Array.prototype.forEach.call($globalScope.querySelectorAll('[data-filter]'), function (el) {
                    if (cls.viewFilter[dom.getXPath(el)] !== el.getAttribute('data-filter')) {
                        // filter for this view has changed
                ...
                for (addrN in changes) {
                    //only this fields need to be refreshed

                    var $els = function () {
                        var domAppearance = dom.findUntilParentExists(changes[addrN][0]);
                        var filtered = [];
                        for (var i in domAppearance) {
                            var kick = false;
                            for (var _e = 0; _e < i; _e++) {
                                if (domAppearance[i].contains(domAppearance[_e])) {
                                    kick = true;
                                    break;
                                }
                            }
        ...
        cls.dispatchEvents = function () {
            ...
            if (this.getAttribute(api.on.attr)) {
                ...
                for (i in events) {
                    ...
                    parts = event.split('->');
                    if (parts.length > 1) {
                        _FinalEvents[parts[0]] = parts[1];
                    } else {
                        _FinalEvents[parts[0]] = parts[0];
                    }
                }
            }
        ...
        cls.bind = function (element) {
            ...
            var factory = function factory(me, event) {
                return function (e, args) {
                    name = dom.getName(me);
                    method = events[name][event];
                    var result = true;
                    if (false !== cls.pre_trigger.call(me, e)) {
                        if (typeof cls.interactions[name] !== 'undefined' && typeof cls.interactions[name][method] !== 'undefined') {
            ...
            function bindevents(el) {
                ...
                for (event in events[name]) {
                    ...
                    if (cls.config.skeleton) {
                        if (typeof skeleton['interactions'][name] === 'undefined') skeleton['interactions'][name] = {};
        ...
        cls.applyMethods = function (el) {
            ...
            el.getValue = function (from, multiple) {
                if (typeof from === 'undefined') from = 'dom';
        ...
        cls.ajax = function (method, url) {
            ...
            _xhr.setup = function (cb) {
                // hacky? maybe
                cb(_xhr);
                return _xhr;
            };
            _xhr.done = function (cb) {
                // hacky? maybe
        ...
        if (cls.view.viewpath && !cls.view.templates_) {
            (function () {
                ...
                for (var v in cls.view.views) {
                    cls.ajax("get", cls.view.viewpath + v + '.' + cls.view.fileextension + '?v=' + (cls.config.debug ? Math.random() : '1')).done(function (v) {
...
//# sourceMappingURL=klaster.es5.dev.js.map
